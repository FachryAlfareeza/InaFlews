...
import { Maximize2, Layers, RefreshCw } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col h-screen bg-[#060c1a] text-slate-100 overflow-hidden">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 relative overflow-hidden min-h-0 min-w-0">
          ...
          <div className="absolute top-3 right-3 z-[1000] flex items-center gap-2">
            ...
            >
              ...
              {isRefreshing ? 'Refreshing...' : `Refresh • ${lastRefresh}`}
            </button>
          </div>
          ...
          <div className="absolute bottom-16 left-1/2 -translate-x-1/2 z-[1000]">
            <div className="flex items-center gap-2 bg-red-950/70 border border-red-800/50 rounded-full px-3 py-1 backdrop-blur-sm">
              ...
              <span className="text-[10px] text-red-300 font-mono font-medium">
                {mockGeoJSON.features.filter(f => (f.properties as any).overallRiskScore > 0.7).length} CRITICAL ZONES ACTIVE — Click hexagon to inspect
              </span>
            </div>
          </div>
        </div>
        ...
        <HexInspector selected={selectedHex} onClose={() => setSelectedHex(null)} />
      </div>
    </div>
  );
}
